...
def Run(args):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
                    ...
                    if accelerator.is_main_process:
                        ...
                        if args.train_text_encoder and os.path.exists(frz_dir):
                            subprocess.call('rm -r '+save_dir +
                                            '/text_encoder/*.*', shell=True)
                            subprocess.call(
                                'cp -f '+frz_dir + '/*.* ' + save_dir+'/text_encoder', shell=True)
                        chkpth = args.Session_dir+"/"+inst+".ckpt"
                        if args.mixed_precision == "fp16":
                            subprocess.call('python /content/diffusers/scripts/convertosdv2.py ' +
                                            save_dir + ' ' + chkpth + ' --fp16', shell=True)
                        else:
                            subprocess.call(
            ...
            if args.external_captions and global_step == args.stop_text_encoder_training and global_step >= 5:
                subprocess.call('mv '+args.captions_dir+' ' +
                                args.captions_dir+'off', shell=True)
    ...
    if accelerator.is_main_process:
        if args.dump_only_text_encoder:
            ...
            pipeline.save_pretrained(args.output_dir)
            if args.train_text_encoder and os.path.exists(frz_dir):
                subprocess.call('mv -f '+frz_dir + '/*.* ' +
                                args.output_dir+'/text_encoder', shell=True)
                subprocess.call('rm -r ' + frz_dir, shell=True)
        ...
        if os.path.exists(args.captions_dir+'off'):
            subprocess.call('mv '+args.captions_dir+'off ' +
                            args.captions_dir, shell=True)
